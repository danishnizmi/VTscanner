...
def sanitize_for_html(text):
    """
    Safely encode a string for HTML output to prevent XSS attacks
...
def get_severity_class(severity: str) -> str:
    ...
    Args:
        severity: The severity level (Critical, High, Medium, Clean, Error)
        
    Returns:
        The CSS class name corresponding to the severity
    """
    severity_classes = {
...
def get_ms_defender_span(status: str) -> str:
    ...
    Args:
        status: Microsoft Defender status (known, unknown, N/A)
...
def has_microsoft_detection(detection_string: str) -> bool:
    ...
    microsoft_patterns = [
        'microsoft:', 'microsoft/', 'trojan:win32', 'trojan.win32', 
...
def generate_html_report(results_list: List[Dict], scan_stats: Dict, output_path: Optional[str] = None, input_filename: str = "IOCs") -> Optional[str]:
    ...
    # Process data - create a copy to avoid modifying the original
    results_list_copy = [result.copy() for result in results_list]
    
    # Sanitize all string values to prevent XSS
    for result in results_list_copy:
        for key, value in result.items():
            if isinstance(value, str):
                result[key] = sanitize_for_html(value)
    
    # Create DataFrame from the sanitized results
    ...
    def get_ms_defender_status(row):
        ...
        # Check detection_names field
        if 'detection_names' in row and isinstance(row['detection_names'], str):
            if has_microsoft_detection(row['detection_names']):
                return 'known'
        
        # Check last_analysis_results if available
        if 'last_analysis_results' in row and isinstance(row['last_analysis_results'], dict):
            for engine, result in row['last_analysis_results'].items():
                if has_microsoft_detection(engine) or (
                    isinstance(result, dict) and 
                    has_microsoft_detection(str(result.get('result', '')))
                ):
                    return 'known'
        
        # Check all fields for Microsoft detections as a fallback
        for key, value in row.items():
            if isinstance(value, str) and key not in ['detection_names', 'ioc', 'ioc_type', 'error']:
    ...
    ioc_type_fig = px.bar(
        ...
        text='Count',
        category_orders={"IOC Type": sorted(df["ioc_type"].unique())}  # Ensure consistent ordering
    )
    ioc_type_fig.update_layout(
        ...
        xaxis=dict(
            ...
            categoryorder='array',
            categoryarray=sorted(df["ioc_type"].unique())
        ),
        yaxis=dict(
    ...
    # Generate HTML for tables with proper copy functionality
    ...
    for idx, row in critical_df.iterrows():
        ...
        if not vt_link.startswith(('http://', 'https://')):
            vt_link = ''
            
        critical_rows += f"""
                    <tr data-ioc-index="{idx}">
                        <td>
    ...
    for idx, row in ms_detection_df.iterrows():
        vt_link = row.get('vt_link', '')
        if not vt_link.startswith(('http://', 'https://')):
            vt_link = ''
    ...
    for idx, row in df.iterrows():
        ...
        all_results_rows += f"""
            <tr class="{ms_class}" data-ioc-type="{row['ioc_type']}" data-severity="{row['severity']}" data-ms-detection="{row['ms_defender']}" data-ioc-index="{idx}">
                <td>
                    <div class="ioc-container">
                        <span class="ioc-value">{row['ioc']}</span>
                        <button class="copy-btn" onclick="copyToClipboard('{row['ioc']}')">
    ...
    # Encode full results data for export
    export_data = []
    for _, row in df.iterrows():
        export_row = {k: v for k, v in row.items() if k != 'last_analysis_results'}
        export_data.append(export_row)
    
    # Create CSV export button functionality
    csv_export_data = json.dumps(export_data)
    
    # Build the HTML report with improved styling for copy button and export functionality
    html_template = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>VirusTotal Scan Results - {sanitize_for_html(Path(input_filename).name)}</title>
    ...
    <body>
        ...
        <div class="container">
            <div class="header">
                <h1><i class="fas fa-shield-virus"></i> VirusTotal Scan Results - {sanitize_for_html(Path(input_filename).name)}</h1>
        ...
        <div class="footer">
            <p>
                ...
                <span class="tooltip">
                    ...
                    <span class="tooltiptext">Report generated at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</span>
                </span>
            </p>
        </div>
    </div>
